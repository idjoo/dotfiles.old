...
widget_defaults = dict(font="TerminessTTF Nerd Font Mono Medium",
                       fontsize=13,
                       padding=2,
                       background=colors[0])
...
def init_widget_list():
    widget_list = [
        widget.Sep(linewidth=0, padding=6, foreground=colors[7]),
        widget.Image(
        ...
        widget.CheckUpdates(
            ...
            mouse_callbacks={
                'Button1':
                lambda: qtile.cmd_spawn(terminal + ' -e doas xbps-install -Suv'
                                        )
            },
...
def init_screens():
    return [
        Screen(top=bar.Bar(
            widgets=init_widgets_top(), size=20, background=colors[0]))
    ]
